...
function OrderConfirmation({ order, restaurantId }: Iprops) {
  ...
  if (!order || !restaurantId) {
    return <Box>nothing here bruh</Box>;
  }
      ...
      <Box className={classes.height}>
        <Box className={classes.center}>
          <Box className={classes.confirmText}>
            <Typography>
              Tack för din beställning, din order är på väg!
            </Typography>
          </Box>
          <Box className={classes.containerStyle}>
            ...
            <Box className={classes.padding}>
              {order.cart.map((item: MenuItem, index: number) => (
                <Box key={index}>
                  <Typography>
                    {item.quantity} {item.title} {item.price * item.quantity} kr
                  </Typography>
                </Box>
              ))}
            </Box>
            ...
            <Box className={classes.textSpacing}>
              <Typography>Betalsätt: {order.paymentType}</Typography>
